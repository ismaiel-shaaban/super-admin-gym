...
const Coaches = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    country_id: '',
    phone_country: '',
    role: USER_ROLE_IDS.COACH,
    birthdate: '',
    gender: '',
  });

  // Fetch coaches and countries on component mount
  useEffect(() => {
    dispatch(fetchUsers({ page: 1, role: USER_ROLE_IDS.COACH }));
  ...
  const handleEditUser = (user) => {
    ...
    setFormData({
      ...
      role: USER_ROLE_IDS.COACH, // Always set as coach for this page
  ...
  return (
    <div className="coaches-page">
      <Card>
        ...
        <Card.Body>
          ...
          <Table responsive striped hover>
            <thead>
              <tr>
                <th>{t('coaches.tableHeaders.id')}</th>
                <th>{t('coaches.tableHeaders.name')}</th>
                <th>{t('coaches.tableHeaders.email')}</th>
                <th>{t('coaches.tableHeaders.phone')}</th>
                <th>{t('coaches.tableHeaders.role')}</th>
                <th>{t('coaches.tableHeaders.createdAt')}</th>
                <th>{t('coaches.tableHeaders.actions')}</th>
              </tr>
            </thead>
            <tbody>
              {users.map((user) => (
          ...
          {users.length === 0 && !loading && (
            <div className="text-center py-4">
              <p className="text-muted">{t('coaches.noCoachesFound')}</p>
            </div>
          )}
            ...
            <div className="d-flex justify-content-between align-items-center mt-4">
              <div className="pagination-info">
                <small className="text-muted">
                  {t('coaches.showingResults').replace('{from}', from || 0).replace('{to}', to || 0).replace('{total}', total || 0)}
                </small>
              </div>
      ...
      <Modal show={showAddModal} onHide={() => setShowAddModal(false)}>
        <Modal.Header >
          <Modal.Title>{t('coaches.modals.addTitle')}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {createError && (
          ...
          <Form>
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>{t('coaches.formLabels.name')}</Form.Label>
                  <Form.Control
                    type="text"
                    name="user_name"
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>{t('coaches.formLabels.email')}</Form.Label>
                  <Form.Control
                    type="email"
                    name="email"
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>{t('coaches.formLabels.phone')}</Form.Label>
                  <Form.Control
                    type="tel"
                    name="phone"
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>{t('coaches.formLabels.country')}</Form.Label>
                  <Form.Select
                    name="country_id"
                    ...
                    required
                  >
                    <option value="">{t('coaches.formLabels.selectCountry')}</option>
            ...
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>{t('coaches.formLabels.birthdate')}</Form.Label>
                  <Form.Control
                    type="date"
                    name="birthdate"
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>{t('coaches.formLabels.gender')}</Form.Label>
                  <Form.Select
                    name="gender"
                    value={formData.gender}
                    onChange={handleInputChange}
                  >
                    <option value="">{t('coaches.formLabels.selectGender')}</option>
                    <option value="male">{t('coaches.formLabels.male')}</option>
                    <option value="female">{t('coaches.formLabels.female')}</option>
                  </Form.Select>
                </Form.Group>
              </Col>
            </Row>
            <Row>
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>{t('coaches.formLabels.password')}</Form.Label>
                  <Form.Control
                    type="password"
                    name="password"
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  <Form.Label>{t('coaches.formLabels.passwordConfirmation')}</Form.Label>
                  <Form.Control
                    type="password"
        ...
        <Modal.Footer>
          ...
          >
            {createLoading ? t('coaches.buttons.loading') : t('coaches.buttons.save')}
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        <Modal.Header >
          <Modal.Title>{t('coaches.modals.editTitle')}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {updateError && (
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    value={formData.password}
                    onChange={handleInputChange}
                    placeholder={t('coaches.formLabels.passwordPlaceholder')}
                  />
                </Form.Group>
              </Col>
              <Col md={6}>
        ...
        <Modal.Footer>
          ...
          >
            {updateLoading ? t('coaches.buttons.loading') : t('coaches.buttons.save')}
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>{t('coaches.modals.deleteTitle')}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {deleteError && (
          ...
          <p>
            {t('coaches.confirmDelete')} <strong>{selectedUser?.user_name || 'Unknown'}</strong>?
            {t('coaches.deleteWarning')}
          </p>
        </Modal.Body>
        <Modal.Footer>
          ...
          >
            {deleteLoading ? t('coaches.buttons.loading') : t('coaches.buttons.delete')}
          </Button>
        </Modal.Footer>
      </Modal>
